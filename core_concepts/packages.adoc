...
* A command: A simple shell command as an executable. *If a git or docker reference is provided, this shell command overrides
the link:https://docs.docker.com/engine/reference/run/#cmd-default-command-or-options[CMD] of the docker container while we
 don't allow the overwriting of entry-point. Otherwise, if no git or docker reference is provided, just the command can
 form an executable. But in this case, the executable can only run on a device environment (not the cloud).* Rapyuta.io
 assumes that all dependencies that are required to run this command are already present in the device runtime where the
 command is expected to run.

[TIP]
When using the bash option with container, the specified command is executed with an invocation of the shell using /bin/bash -c.
But make sure it does contain the bash.
