...
The package creator may want to alter certain behavior of the program based on user provided values at run-time.
These are modelled and  injected into the IO Package execution environment -
 ...
 and can be read by the code of any executable through standard environment lookup techniques (eg in python one could use `os.environ`)
