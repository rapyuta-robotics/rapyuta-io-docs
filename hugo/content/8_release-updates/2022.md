...
| ------- | ---- | ----------- |
| [Rapyuta IO Metrics Collector](/5_deep-dives/54_tooling-and-debugging/545_user_metrics/#rapyuta-io-metrics-collector) | Improvement | By default, the **Rapyuta IO Metrics Collector** is now available on all ROS device deployments. |

### Bug Fixes

* When a user searches for a file name in the table view of the Manage Device Logs page, the search results reflected only on the current page, the search context was lost when the user navigated to a different page. This issue is now fixed.
