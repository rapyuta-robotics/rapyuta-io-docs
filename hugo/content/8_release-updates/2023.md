...
| ------- | ---- | ----------- |
| Config command | New | You can now use the config command to set custom configurations on the device. |
...
| Package Version  | Improvement | You now have the option to switch the package versions if a package has more than one version.|
