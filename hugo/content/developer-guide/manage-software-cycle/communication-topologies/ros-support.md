...
- [Multi-robot Support](#multi-robot-support)
    ...
    - [Targeted: Topics, Actions and Services](#targeted-topics-actions-and-services)
    - [Inbound Targeted: Topics, Actions, Services](#inbound-targeted-topics-actions-services)
...
#### Targeted: Topics, Actions and Services
...
Suppose a group of robots wants to send requests to a service that is
running on the central controller. In this scenario, the controller
will make the service targeted for each of the robots by their
identities (aliases).
For example, the controller runs a service, */controller_manager/load_controller*,
and is made targeted for a group of robots. When a robot say robotA calls the
service, the controller invokes the */robotA/controller_manager/load_controller*
service targeted at robotA.

A targeted service is expressed as a mapping from /service to /robot-alias/service.

Similarly, a targeted action is expressed as a mapping from /action to /robot-alias/action.
...
#### Inbound Targeted: Topics, Actions, Services
This serves a purpose identical to the targeted topics, actions and services
albeit in a dependant deployment scenario.
