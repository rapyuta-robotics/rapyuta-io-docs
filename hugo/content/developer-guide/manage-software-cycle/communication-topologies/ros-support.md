...
- [Multi-robot Support](#multi-robot-ros-communication)
  ...
  - [Dynamic multi-robot communication semantics](#dynamic-multi-robot-communication-semantics)
      - [ROS Environment Aliases](#ros-environment-aliases-runtime-indentitiy-assignment)
      - [Discovering Peers in an Ensemble](#discovering-peers-in-an-ensemble)
      - [Scoping](#scoping-auto-prefix-namespace-by-self-identity)
      - [Targeting](#targeting-auto-prefix-namespace-unwrapping-for-peers)
...
The rapyuta.io platform offers an elegant solution for multiple robot communication as a primary feature. 
In the rapyuta paradigm, the component of each package is treated as an isolated ROS environment.
...
To illustrate a scenario involving multiple robots we turn to an example involving the world's favourite sport - soccer. 
Similar topologies are often relevant in real-world applications of robots such as warehouses where a 'coordinator' controls multiple AMR/AGVs 
...
* A player moves to a position when he recieves a message on the */move* topic
* A player publishes its pose and location through */odom*.

Now if the coach (controller) needs to use this information from all players(robots) in the field.
To deal with multiple players(robots) it is necessary to create a convention that allows him to specifically 
access information about one specific player(robot) and issue commands to one specific player(robot). 

In the ROS community common approach used in multirobot communcation scenarios is to __prefix or namespace__ the interfaces (topics/services/actions) __by a unique identity__ typically the name of the robot. 
...
* wants to seek odom from _robot A_, it must expilicitly subscribe to */robotA/odom* that is published by _robot A_  (or */robotB/odom*
and */robotC/odom* from robot B and robot C respectively)

This is typically achieved using carefully crafted launchfiles using remaps (eg /move to /robot_A/move), conditionals(eg unless ns!="" or if robot_name=="robot_A"), arguments(robot_name:=robot_A) and namespaces(eg <node ns=robot_A>). This mandates the delicate arrangemnt of files is frozen while building the software and consistently distributed to all involved agents. As the needs/software change and number of variables & robots increase this approach becomes increasingly error prone.
...
It is prudent and avoid complex hardcoded logic in launchfiles that lives with the source-code or binary and automatically add/remove prefixes to ROS interfaces(topics/services/actions) .
Additionally it is significantly more flexible/dynamic to assign and and use deploy-time identities than hard-coded robot names. 

The process of assigning identity to a robot and the mechanisms to consume/discover identitiies of all alive robots is described in the [ROS Environment Alises](#ros-environment-aliases-runtime-indentitiy-assignment) section.

The mechanisms and features offered by the platform to deal with automatic prefix addition and removal is described in the [scoping]() and [targeting]() sections



#### ROS Environment Aliases - runtime indentitiy assignment 
...
(more on this in the next section) therby assigning a stable identitiy to that deployment for the period of its lifetime.

This __alias__ defined at deploy-time __uniquely identifies__ the robot in the __scope of an ensemble__ of connected peers

All __components in the same package__ (each potentially deployed in a physically different device/location) or __linked dependent deployments__ are considered a __part of an ensemble__
...
Rapyuta automatically exposes a latched ROS topic
...
In this configuration, a user may declare a topic/service/action as _**scoped**_ by selecting the "Scoped" option. 

This indicates that when a component is deployed its local ros interfaces (topic/service/action) get __automatically prefixed/namespaced by its own dynamic identity__ (its own "ROS environemnt alias") __as seen by all other other robots/peers__ in the ensemble in thier respective ROS graph.
...
When a package allows for [inbound ROS interfaces](/developer-guide/create-software-packages/ros-support/#inbound-interfaces) the user must provide hints to leverage the auomatic targeting feature
...
As the platform follows a provider only semantic determining this is straightforward for *_scoped_* as it is based on the identitiy of the deployment itself.
Matters get more complicated for targeting as this depends on the identitiy or other peers. When a package is the first to be deployed (or root in any particular subtree of dependants) it becomes necessary to provide a hint to indicate that the interfaces will part-take in communication topologies that require the presence of a stable unique identity.
