...
... In this case, we will use the executable `talker` from the io_tutorials git repository. Therefore, select git and add this URL: https://github.com/rapyuta/io_tutorials
...
... The code is already on the device. We simply have to tell rapyuta.io what to run on the device:
    ...
    source ~/catkin_ws/devel/setup.bash && roslaunch listener listener.launch
